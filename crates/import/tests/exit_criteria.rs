...
fn importing_a_folder_with_known_dupes_and_a_raw_jpeg_pair_produces_a_correct_queue_and_pairing_table()
 {
    ...
    std::fs::copy(
        source_dir.path().join("dup_a.jpg"),
        source_dir.path().join("dup_b.jpg"),
    )
    .unwrap();
    ...
    let near_2_img =
        image::imageops::resize(&cropped, 64, 64, image::imageops::FilterType::Nearest);
    ...
    assert_eq!(
        dup_image_ids.len(),
        1,
        "both exact-duplicate filenames must resolve to the same images row"
    );
    let dup_image_id = dup_image_ids[0];
    let dup_source_count: i64 = conn
        .query_row(
            "SELECT COUNT(*) FROM image_sources WHERE image_id = ?1",
            [dup_image_id],
            |row| row.get(0),
        )
        .unwrap();
    assert_eq!(
        dup_source_count, 2,
        "the exact-duplicate image must have more than one image_sources row"
    );
    ...
    assert_eq!(
        queue_rows.len(),
        1,
        "expected exactly one dedupe_review_queue row, got {queue_rows:?}"
    );
    ...
    assert_eq!(
        *distance, observed_near_distance as i64,
        "queued distance must match the real computed Hamming distance"
    );
    ...
    assert_eq!(
        control_in_queue, 0,
        "the control image must never appear in the review queue"
    );

    // ── RAW+JPEG pairing ──────────────────────────────────────────────
    let raw_id: i64 = conn
        .query_row(
            "SELECT image_id FROM image_sources WHERE original_filename = 'IMG_0001.cr2'",
            [],
            |row| row.get(0),
        )
        .unwrap();
    let raw_jpeg_id: i64 = conn
        .query_row(
            "SELECT image_id FROM image_sources WHERE original_filename = 'IMG_0001.jpg'",
            [],
            |row| row.get(0),
        )
        .unwrap();
    ...
    assert_eq!(
        raw_width, None,
        "a recognized-RAW file must have NULL width (no pixel decode attempted)"
    );
    assert_eq!(raw_height, None);
    assert_eq!(
        raw_perceptual_hash, None,
        "a recognized-RAW file must have NULL perceptual_hash"
    );
    assert_eq!(
        raw_conversion_status, "not_applicable",
        "RAW has no defined §4 conversion path"
    );
    ...
    assert_eq!(
        pair_rows.len(),
        1,
        "expected exactly one raw_jpeg_pairs row, got {pair_rows:?}"
    );
    assert_eq!(
        pair_rows[0],
        (raw_id, raw_jpeg_id),
        "the pairing must link the RAW image to its JPEG counterpart"
    );
    ...
    assert_eq!(
        raw_in_queue, 0,
        "the RAW file must never appear in the perceptual review queue"
    );
    let raw_jpeg_side_in_queue: i64 = conn
        ...
        .unwrap();
    assert_eq!(
        raw_jpeg_side_in_queue, 0,
        "the RAW file's paired JPEG must not independently land in the review queue either"
    );
}
