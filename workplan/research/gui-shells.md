...
**For LumenVault**: zero-network is a standing non-negotiable constraint (see `workplan/MAP.md`) and the central view *is* the 100k+ grid. That weighs toward a pure-Rust shell (egui is the most battle-tested at large-data-scale via Rerun; Slint if its license and better a11y/i18n matter more than ecosystem size) — unless UI velocity is valued enough to accept "prove WebView2 is neutered by config" as an ongoing burden. That call belongs to [Choose the GUI shell](../tickets/007-choose-gui-shell.md), not this ticket.
